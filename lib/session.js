 ...
 *
 * @param {Object} lookup - must contain username and password
...
exports.signin = function(lookup, req, res, onSuccess, onFail) {
	...
	if ('string' === typeof lookup.username && 'string' === typeof lookup.password) {

		// match username address and password
		User.model.findOne({ username: lookup.username }).exec(function(err, user) {
			if (user) {

...
function routes(app) {
	...
	if (this.get('auth') === true) {
		...
		if (!this.get('account_manager url')) {
			this.set('account_manager url', '/keystone/account_manager');
		}
		...
		app.all('/keystone/account_manager', require('../../routes/views/account_manager'));
